...
Hands-on examples for **Git Basics for Course Work**: concepts, init/clone, status/diff, add/commit/log, .gitignore, remote/undo, tags, and stash.
...
Or create your own: `mkdir my_repo && cd my_repo && git init`. See `init_clone/README.md` and `init_clone/commands_to_try.txt` for clone commands.
...
## 9. Branch rename and delete (`branch_rename/`)
...
See `branch_rename/README.md` for -m (rename) and -d/-D (delete).

---

## 10. git log options (`log_options/`)

Practice **viewing history** with different `git log` options: oneline, graph, patch, name-only, and filtering.

**Layout**: `commands_to_try.txt`.

**Try these** (in a Git repo with some commits — e.g. ~/unix_practice/git_demo):
...
See `log_options/README.md` for --oneline, --graph, -p, --name-only, and filtering.
